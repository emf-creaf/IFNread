...
readPiesMenoresIFN2<-function(prov, DBFdir = "DBF",subsetVars=TRUE){
  ...
  if(subsetVars){
    ...
    regTreeDataIFN2 <- regTreeDataIFN2[,c(which(n=="ID"),which(n=="ESPECIE"),which(n=="NUMERO"), which(n=="DG"),
                                          which(n=="ALTUMED"))]
    names(regTreeDataIFN2) <- c("ID","Especie","N","DG","Ht")
  }
  regTreeDataIFN2$OIF2<-"-1"
  regTreeDataIFN2$REG<-1 #Pies menores
